...
static int process_free_program_data(struct process* process){
    ...
    switch(process->filetype){
        case PROCESS_FILE_TYPE_BIN: res = process_free_binary_data(process); break;
        case PROCESS_FILE_TYPE_ELF: res = process_free_elf_data(process); break; 
        default: res = -EINVARG;
    }
    return res;
}
...
int process_terminate(struct process* process){
    ...
    // free process stack
    kfree(process->stack);
    // free task of the process
    task_free(process->task);
    // unlink process from processes array
